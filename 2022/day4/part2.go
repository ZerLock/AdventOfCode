...
func main() {
	...
	for _, pair := range pairs {
		...
		if (firstElf[0] >= secondElf[0] && firstElf[0] <= secondElf[1]) || (firstElf[1] <= secondElf[1] && firstElf[1] >= secondElf[0]) ||
			(secondElf[0] >= firstElf[0] && secondElf[0] <= firstElf[1]) || (secondElf[1] <= firstElf[1] && secondElf[1] >= firstElf[0]) {
			count++
		}
	}
	fmt.Println(count)
